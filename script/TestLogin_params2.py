...
import logging
import app
from utils import assert_utils, DButils, read_params_data
...
class TestLogin(unittest.TestCase):
    ...
    # 测试获取图片验证码脚本
    @parameterized.expand(read_params_data("imgCode.json"))
    def test01_get_img_code(self,title,data_type,status_code):
        # 准备测试数据
        if data_type == "float":
            r = random.random()
        elif data_type == "int":
            ...
            r = random.sample("abcdefghilmn",8)
    ...
    # 注册脚本
    @parameterized.expand(read_params_data("register2.json"))
    def test09_register(self,title,phone,pwd,imgCode,smsCode,dyServer,initePhone,scode,status,desc):
        # 1、获取图片验证码
        # 准备测试数据
        r = random.random()
        # 调用接口类中定义的方法，发送请求并接收响应
        ...
        # 准备测试数据
        # 调用接口API方法，发送请求并接收响应
        response = self.login_api.register(self.session,phone,pwd,imgCode,smsCode,dyServer,initePhone)
        logging.info("response = {}".format(response.json()))
        # 对响应结果进行断言
        assert_utils(self,response,scode,status,desc)
