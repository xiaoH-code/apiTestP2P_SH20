...
import logging
import app
from utils import assert_utils, DButils, read_imgCode_data,read_register_data
...
class TestLogin(unittest.TestCase):
    ...
    # 测试获取图片验证码脚本
    @parameterized.expand(read_imgCode_data())
    def test01_get_img_code(self,data_type,status_code):
        # 准备测试数据
        if data_type == "float":
            r = random.random()
        elif data_type == "int":
    ...
    def test09_register(self,data,status_code,status,desc):
        # 1、获取图片验证码
        # 准备测试数据
        r = random.random()
        # 调用接口类中定义的方法，发送请求并接收响应
        ...
        # 准备测试数据
        # 调用接口API方法，发送请求并接收响应
        response = self.login_api.register(self.session,data.get("phone"),data.get("password"),data.get("verifycode"),data.get("phone_code"),data.get("dy_server"),data.get("invite_phone"))
        logging.info("response = {}".format(response.json()))
        # 对响应结果进行断言
        assert_utils(self,response,status_code,status,desc)
